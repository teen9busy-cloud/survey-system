...
export default function FacultySurveyPage() {
  ...
  const totalSteps = FACULTY_SURVEY.length;
  const currentPart = FACULTY_SURVEY[currentStep - 1];
  ...
  const handlePhoneBlur = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/survey/check-duplicate?type=faculty&phone=${phone}`);
